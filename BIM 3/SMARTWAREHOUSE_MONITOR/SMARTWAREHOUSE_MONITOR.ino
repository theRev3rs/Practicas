...
SoftwareSerial BT(SerialRXD, SerialTXD);
void setup() {
  Serial.begin(9600);
  Serial.begin(9600);
...
void loop() {
  if((Serial.available()>0) && (Serialconectado == false)){
  ...
  if(inicio== String("Conectado")){
    ...
    while((Serial.available()>0) && (Serialconectado == true)){
    entrada_Serial = Serial.readStringUntil('\n'); 
    Serial.println(entrada_Serial);
    delay(300);
    cantidad = Serial.parseInt();
    
    if(entrada_Serial == String("Refrescar")){
      stock1 = 10;
      stock1 = 10;
      stock1 = 10;
      }
      if(entrada_Serial == String("Desconectado")){
  ...
  void actuador_lineal(){
    if(entrada_Serial == String("Producto1")){
    if(cantidad <= stock1){
      pedido = true;
//      Funcion de la banda
      ...
      for(int i = 0 ; i < pos1; i++){
        ...
        Serial.println();
//    act_line.step(pos1); //Moverse a la posicion del entrada_Serial 1
    delay(100);
    for(int i = 0; i < cantidad; i++){  //Realiza el empuje segun cuantos entrada_Serials se necesiten
...
//    Funcion para apagar la banda
    ...
    if(entrada_Serial == String("Producto2")){
    if(cantidad <= stock2){
      pedido = true;
//      Funcion de la banda
      Serial.println("Despachando");
